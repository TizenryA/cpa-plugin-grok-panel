...
.panel-tabs{display:flex;gap:.55vw;padding:.55vh .55vw;margin-bottom:1.35vh;border:.08vw solid var(--glass-line);border-radius:var(--radius-xl);background:linear-gradient(145deg,rgba(255,255,255,.78),rgba(255,255,255,.46));box-shadow:var(--shadow-card),var(--highlight);backdrop-filter:blur(1.8vw) saturate(1.45);-webkit-backdrop-filter:blur(1.8vw) saturate(1.45);position:sticky;top:.7vh;z-index:20}.panel-tab{flex:1;border-color:transparent;background:transparent;box-shadow:none;color:var(--muted);font-weight:680}.panel-tab:hover:not(:disabled){background:rgba(255,255,255,.54);color:var(--ink);transform:none}.panel-tab.active{color:#fff;background:linear-gradient(135deg,var(--accent),var(--accent-2));border-color:rgba(255,255,255,.42);box-shadow:0 .5vw 1.4vw rgba(37,99,235,.24),inset 0 .08vw 0 rgba(255,255,255,.4)}.panel-view{display:none;animation:panelReveal .22s cubic-bezier(.2,.75,.25,1)}.panel-view.active{display:block}.panel-view>.panel:last-child{margin-bottom:0}@keyframes panelReveal{from{opacity:0;transform:translateY(.7vh);filter:blur(.18vw)}to{opacity:1;transform:translateY(0);filter:blur(0)}}
@media (prefers-color-scheme:dark){.panel-tabs{background:linear-gradient(145deg,rgba(19,19,22,.82),rgba(5,5,7,.64));border-color:rgba(255,255,255,.14)}.panel-tab:hover:not(:disabled){background:rgba(255,255,255,.08)}}
...
<div class="help-text">删除和清理需要再次点击同一按钮确认。插件在管理中心 iframe 中通常读不到“记住密码”，请在下方填写管理密钥后保存。「Responses 实测」仅手动连接固定的 Grok 官方 HTTPS 地址；不会信任 auth 里的任意地址，不会改变健康状态、连续失败次数或删除资格。</div>
...
<button id="testResponsesBtn" title="手动连接固定的 Grok 官方 Responses 地址；结果不参与自动清理">Responses 实测</button>
...
function measuredLimits(x){var r=x&&x.responses_limits||{};return r.available===true?r:null}
function effectiveTokenLimit(x){var r=measuredLimits(x);return r&&r.token_available===true&&Number(r.token_limit)>0?Number(r.token_limit):tokenLimit()}
function effectiveTokenUsed(x){var r=measuredLimits(x);return r&&r.token_available===true?Math.max(0,Number(r.token_used)||0):(Number(x&&x.success)||0)*tokensPerReq()}
function usagePct(x){var tl=effectiveTokenLimit(x);var used=effectiveTokenUsed(x);return tl>0?Math.max(0,Math.min(999,used/tl*100)):0}
...
async function testResponsesFormat(){var emails=getSelectedEmails();if(!emails.length){setFeedback('请先选择账号。','warn');return}var rows=emails.map(accountByEmail).filter(function(x){return x&&x.auth_index});if(!rows.length){setFeedback('选中账号缺少 auth_index，无法读取 auth 凭证。','warn');return}if(busy)return;if(!readCPAConnection()){setFeedback('Responses 实测失败：没有管理密钥。','error');updateAuthBadge();return}setBusy(true);var ok=0,failed=0,details=[];try{for(var i=0;i<rows.length;i++){var x=rows[i];var email=getEmail(x);setFeedback('Responses 实测：'+(i+1)+' / '+rows.length+' · '+email+'（仅访问 Grok 官方地址，不参与自动清理）','');try{var data=await managementPluginPost('probe-responses',{auth_index:x.auth_index});var r=Array.isArray(data.records)?data.records[0]:{};if(r.ok===true){ok++;var preview=r.output_preview||'';var lim=r.responses_limits||{};var tokenQuota=lim.token_available===true?('Token '+fmt(lim.token_remaining)+' / '+fmt(lim.token_limit)):'Token 限额未完整返回';var requestQuota=lim.request_available===true?('请求 '+fmt(lim.request_remaining)+' / '+fmt(lim.request_limit)):'请求限额未完整返回';var quota=' · '+tokenQuota+' · '+requestQuota;details.push(email+': OK'+quota+(preview?(' → '+String(preview).slice(0,40)):'') )}else{failed++;details.push(email+': FAIL '+(r.error||r.reason||('HTTP '+(r.http_status||'?'))))}}catch(e){failed++;details.push(email+': '+e.message)}}await fetchData(false,true);var msg='Responses 实测完成：成功 '+ok+' / 失败 '+failed+'（共 '+rows.length+'）。限额来自官方响应头；结果不改变健康状态和删除资格。';if(details.length)msg+=' 详情：'+details[0]+(details.length>1?' …':'');setFeedback(msg,failed?'warn':'ok')}catch(e){setFeedback('Responses 实测失败：'+e.message,'error')}finally{setBusy(false)}}function manualCheckSelected(){var emails=getSelectedEmails();if(!emails.length){setFeedback('请先选择账号。','warn');return}runHealthCheck(emails,'手动检查选中 '+emails.length+' 个',false)}
