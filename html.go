...
<script>
/*
Frontend v1.1.2 same-origin endpoint contract for a matching backend.
...
*/
var settingsKey='grok-panel-v1.1.2-settings';
var allData=[];
var lastData=null;
var selected={};
var confirmUntil={};
...
function parseJsonText(text,endpoint){try{return text?JSON.parse(text):{}}catch(e){var low=String(text||'').toLowerCase();if(low.indexOf('<!doctype')>=0||low.indexOf('<html')>=0)throw new Error('操作端点 '+endpoint+' 未启用：当前后端返回了面板页面，请升级插件后端 v1.1.2 或注册该管理路由。');throw new Error('操作端点 '+endpoint+' 返回非 JSON：'+String(text||'').slice(0,90))}}
