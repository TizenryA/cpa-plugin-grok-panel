...
<script>
/*
Frontend v1.1.10 same-origin endpoint contract for a matching backend.
...
*/
var settingsKey='grok-panel-v1.1.10-settings';
...
function parseJsonText(text,endpoint){try{return text?JSON.parse(text):{}}catch(e){var low=String(text||'').toLowerCase();if(low.indexOf('<!doctype')>=0||low.indexOf('<html')>=0)throw new Error('操作端点 '+endpoint+' 未启用：当前后端返回了面板页面，请升级插件后端 v1.1.10 或注册该管理路由。');throw new Error('操作端点 '+endpoint+' 返回非 JSON：'+String(text||'').slice(0,90))}}
...
function renderTable(){var tbody=byId('tableBody');if(!tbody)return;var f=getFilteredData();var meta=getPageMeta(f);renderPager(meta);tbody.innerHTML='';meta.rows.forEach(function(x,i){var email=getEmail(x);var absIndex=meta.start+i;var key=makeRowKey(x,absIndex);var su=Number(x.success)||0;var fa=Number(x.failed)||0;var et=su*tokensPerReq();var pct=usagePct(x);var bc=pct>80?'danger':pct>50?'warn':'';var typ=classifyType(x);var h=deriveHealth(x);var prot=protectReason(x);var armed=isArmed('delete:'+key);var disabled=busy||!email||!!prot;var tr=document.createElement('tr');tr.className='row-'+h.key;var protectedText=prot?'<div class="cell-sub">⚠ '+esc(prot)+'</div>':'';var healthDetail=h.detail?'<div class="cell-sub">'+esc(h.detail)+'</div>':'';var healthHtml='<span class="health-indicator"><span class="health-dot '+esc(h.key)+'"></span><span class="tag '+esc(h.key)+'">'+esc(h.label)+'</span></span>'+healthDetail;tr.innerHTML='<td><input type="checkbox" class="row-select" data-key="'+esc(key)+'" data-email="'+esc(email)+'" '+(selected[key]?'checked':'')+' '+(!email||busy?'disabled':'')+'></td><td>'+(absIndex+1)+'</td><td class="email-cell">'+esc(email||'?')+protectedText+'</td><td><span class="tag '+esc(typ.key)+'">'+esc(typ.label)+'</span></td><td>'+healthHtml+'</td><td><span class="tag '+esc(statusKey(x))+'">'+esc(statusLabel(statusKey(x)))+'</span><div class="cell-sub">'+esc(getStatus(x)||'-')+'</div></td><td>'+fmt(su)+'</td><td>'+(fa>0?'<span class="red-text">'+fmt(fa)+'</span>':'0')+'</td><td>'+fmt(et)+'</td><td><div class="cell-sub">'+pct.toFixed(1)+'%</div><div class="bar-container small"><div class="bar-fill '+bc+'" style="width:'+meterWidth(pct,'row')+'"></div></div></td><td><div class="actions-cell"><button data-act="check" data-key="'+esc(key)+'" '+(busy||!email?'disabled':'')+'>检查</button><button data-act="delete" data-key="'+esc(key)+'" '+(disabled?'disabled':'')+' class="'+(armed?'armed':'')+'">'+(armed?'确认删除':'删除')+'</button></div></td>';tbody.appendChild(tr)});if(!meta.rows.length)tbody.innerHTML='<tr><td colspan="11" class="muted" style="padding:3vh;text-align:center">没有匹配的账号</td></tr>';renderSummary(f);updateToolbarState(meta.rows)}
