...
<div class="help-text">删除和清理需要再次点击同一按钮确认。插件在管理中心 iframe 中通常读不到“记住密码”，请在下方填写管理密钥后保存。「Responses 测试」会读取选中账号 auth 文件中的 access_token / base_url / headers，向上游 {base_url}/responses 发送 model=grok-4.5 的轻量探测（不会把凭证返回浏览器）。</div>
...
<button id="testResponsesBtn" title="用选中账号 auth 凭证探测上游 /v1/responses">Responses 测试</button>
...
*/
var settingsKey='grok-panel-v1.1.23-settings';
...
async function testResponsesFormat(){var emails=getSelectedEmails();if(!emails.length){setFeedback('请先选择账号。','warn');return}var rows=emails.map(accountByEmail).filter(function(x){return x&&x.auth_index});if(!rows.length){setFeedback('选中账号缺少 auth_index，无法读取 auth 凭证。','warn');return}if(busy)return;if(!readCPAConnection()){setFeedback('Responses 测试失败：没有管理密钥。插件需通过管理接口读取 auth 文件凭证。','error');updateAuthBadge();return}setBusy(true);var ok=0,failed=0,details=[];try{for(var i=0;i<rows.length;i++){var x=rows[i];var email=getEmail(x);setFeedback('Responses 测试：'+(i+1)+' / '+rows.length+' · '+email+'（auth 凭证探测 /responses）','');try{var data=await managementPluginPost('probe-responses',{auth_index:x.auth_index,model:'grok-4.5',input:'What is 2+2? One word.'});var results=applyCheckResult(data);var r=(Array.isArray(data.records)?data.records[0]:null)||results[0]||{};if(r.ok===true||r.health==='healthy'){ok++;var preview=r.output_preview||r.outputPreview||'';details.push(email+': OK'+(preview?(' → '+String(preview).slice(0,40)):'')+(r.endpoint?(' @ '+r.endpoint):''))}else{failed++;details.push(email+': FAIL '+(r.error||r.reason||('HTTP '+(r.http_status||r.httpStatus||'?'))))}renderAll()}catch(e){failed++;details.push(email+': '+e.message)}}await fetchData(false,true);var msg='Responses 测试完成：成功 '+ok+' / 失败 '+failed+'（共 '+rows.length+'）。';if(details.length)msg+=' 详情：'+details[0]+(details.length>1?' …':'');setFeedback(msg,failed?'warn':'ok')}catch(e){setFeedback('Responses 测试失败：'+e.message,'error')}finally{setBusy(false)}}function manualCheckSelected(){var emails=getSelectedEmails();if(!emails.length){setFeedback('请先选择账号。','warn');return}runHealthCheck(emails,'手动检查选中 '+emails.length+' 个',false)}
