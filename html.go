...
async function testResponsesFormat(){var emails=getSelectedEmails();if(!emails.length){setFeedback('请先选择账号。','warn');return}var rows=emails.map(accountByEmail).filter(function(x){return x&&x.auth_index});if(!rows.length){setFeedback('选中账号缺少 auth_index，无法读取 auth 凭证。','warn');return}if(busy)return;if(!readCPAConnection()){setFeedback('Responses 实测失败：没有管理密钥。','error');updateAuthBadge();return}setBusy(true);var ok=0,failed=0,details=[];try{for(var i=0;i<rows.length;i++){var x=rows[i];var email=getEmail(x);setFeedback('Responses 实测：'+(i+1)+' / '+rows.length+' · '+email+'（仅访问 Grok 官方地址，不参与自动清理）','');try{var data=await managementPluginPost('probe-responses',{auth_index:x.auth_index});var r=Array.isArray(data.records)?data.records[0]:{};if(r.ok===true){ok++;var preview=r.output_preview||'';details.push(email+': OK'+(preview?(' → '+String(preview).slice(0,40)):'') )}else{failed++;details.push(email+': FAIL '+(r.error||r.reason||('HTTP '+(r.http_status||'?'))))}}catch(e){failed++;details.push(email+': '+e.message)}}var msg='Responses 实测完成：成功 '+ok+' / 失败 '+failed+'（共 '+rows.length+'）。结果仅供诊断，不改变健康状态和删除资格。';if(details.length)msg+=' 详情：'+details[0]+(details.length>1?' …':'');setFeedback(msg,failed?'warn':'ok')}catch(e){setFeedback('Responses 实测失败：'+e.message,'error')}finally{setBusy(false)}}function manualCheckSelected(){var emails=getSelectedEmails();if(!emails.length){setFeedback('请先选择账号。','warn');return}runHealthCheck(emails,'手动检查选中 '+emails.length+' 个',false)}
