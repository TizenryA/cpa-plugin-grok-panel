...
const (
	defaultResponsesBaseURL = "https://cli-chat-proxy.grok.com/v1"
...
func extractAuthCredentials(raw json.RawMessage) authCredentials {
	...
	walk = func(v any, path string) {
		...
		case map[string]any:
			for k, val := range x {
				...
				case "baseurl", "apiurl", "endpoint":
					if s, ok := val.(string); ok && strings.TrimSpace(s) != "" {
						creds.BaseURL = strings.TrimSpace(s)
					}
				case "headers":
					if m, ok := val.(map[string]any); ok {
						for hk, hv := range m {
							if s, ok := hv.(string); ok && strings.TrimSpace(s) != "" {
	...
	if strings.TrimSpace(creds.BaseURL) == "" {
		creds.BaseURL = defaultResponsesBaseURL
	}
	return creds
}

func responsesEndpointURL(baseURL string) string {
...
func handleProbeResponses(req managementRequest, method string) ([]byte, error) {
	...
	checkRec := updateHealthMemory(file, classification, evaluation, settings, now, true, true, nil)
	record.Health = checkRec.Health
	record.Reason = checkRec.Reason
	record.ExplicitStatusCode = checkRec.ExplicitStatusCode
	record.InvalidStreak = checkRec.InvalidStreak
	record.Protected = checkRec.Protected
	record.DeleteEligible = checkRec.DeleteEligible
	record.DeleteIntent = checkRec.DeleteIntent
	record.RuntimeProbeOK = true
	record.LastCheckedAt = checkRec.LastCheckedAt
	if probeErr != nil {
		record.OK = false
...
func probeResponsesAPI(endpoint string, creds authCredentials, model, prompt string) (int, string, error) {
	...
	client := &http.Client{Timeout: responsesProbeTimeout}
